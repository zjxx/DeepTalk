...
trap 'rm -f "$connect_out1" "$connect_err1"  \
               "$disconnect_out1" "$disconnect_err1"  2>/dev/null' EXIT
...
if [ $status1 -eq 0 ] ; then
    ...
    echo "尝试使用令牌 'null' 断开用户 '$USER1_ID' 的连接..."
    curl -v -X POST -H "Content-Type: application/json" \
         -d "{\"userId\": \"$USER1_ID\", \"sessionId\": \"null\"}" \
...
#    cat "$disconnect_err1"
    ...
    if [ $d_status1 -ne 0 ] ; then
        echo "⚠️ 警告: 断开连接命令可能已失败。请检查上面的输出。"
